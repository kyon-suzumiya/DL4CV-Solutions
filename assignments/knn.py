...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.view(x_train.shape[0], -1)
    x_test_flat = x_test.view(x_test.shape[0], -1)
    x_train_sq = torch.sum(x_train_flat * x_train_flat, dim=1, keepdim=True)
    x_test_sq = torch.sum(x_test_flat * x_test_flat, dim=1)
    xy = torch.mm(x_train_flat, x_test_flat.t())
    dists = x_train_sq + x_test_sq.unsqueeze(0) - 2 * xy
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
