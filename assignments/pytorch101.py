...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    row_indices = torch.arange(x.size(0)-1,-1,-1)
    y = x[row_indices]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    num_elements = len(x)
    y = torch.zeros(num_elements, num_classes, dtype=torch.float32)
    y.scatter_(1, x_tensor.unsqueeze(1), 1.0)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M, N = x.shape
    col_means = x.sum(dim=0) / M
    col_variance = ((x - col_means) ** 2).sum(dim=0) / M
    col_std = torch.sqrt(col_variance)
    col_std_safe = torch.where(col_std == 0, torch.ones_like(col_std), col_std)
    y = (x - col_means) / col_std_safe
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    cum_lengths = torch.cumsum(ls, dim=0)
    start_indices = torch.cat([torch.tensor([0], device=ls.device), cum_lengths[:-1]])
    cum_sum = torch.cumsum(all_values, dim=0)
    end_sums = cum_sum[cum_lengths - 1]
    start_sums = torch.cat([torch.tensor([0.0], device=ls.device), cum_sum[start_indices[1:] - 1]])
    sums = end_sums - start_sums
    y = sums / ls.float()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    X, indices = torch.unique(inverse, return_inverse=False)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
